    ...
    **kwargs: Any,
    ...
    while retries <= max_retries:
        try:
            ...
            # Squid ACL denials are 403 HTML pages (often X-Squid-Error).
            # Origin 401/403 forwarded through Squid still carry Via: squid;
            # treating those as SSRF hides the real auth/app error.
            if response.status_code in (401, 403):
                server_header = response.headers.get("server", "").lower()
                via_header = response.headers.get("via", "").lower()
                squid_error = response.headers.get("x-squid-error", "")
                content_type = response.headers.get("content-type", "").lower()
                squid_identified = "squid" in server_header or "squid" in via_header
                squid_generated = bool(squid_error) or (
                    "text/html" in content_type and "application/json" not in content_type
                )
                if squid_identified and squid_generated:
                    response.close()
                    raise ToolSSRFError(
